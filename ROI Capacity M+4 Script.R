...
temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=('Consumption Checks'!O", ROW, "*SUMIFS('CRM REPORT'!$M:$M,'CRM REPORT'!$L:$L,'En & Imp Split'!$B", ROW+2, ",'CRM REPORT'!$J:$J,\"FQMCC\")*'SHADOW SETTLED'!$", toupper(letters[which_column]), "$2)*0.8648"))
}
writeFormula(mywb, EIS, temp, startRow = 4, startCol = 5)

temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=('Consumption Checks'!P", ROW, "*SUMIFS('CRM REPORT'!$M:$M,'CRM REPORT'!$L:$L,'En & Imp Split'!$B", ROW+2, ",'CRM REPORT'!$J:$J,\"FQMCC\")*'SHADOW SETTLED'!$", toupper(letters[which_column]), "$2)*0.8648"))
}
writeFormula(mywb, EIS, temp, startRow = 4, startCol = 6)

temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=('Consumption Checks'!Q", ROW, "*SUMIFS('CRM REPORT'!$M:$M,'CRM REPORT'!$L:$L,'En & Imp Split'!$B", ROW+2, ",'CRM REPORT'!$J:$J,\"FQMCC\")*'SHADOW SETTLED'!$", toupper(letters[which_column]), "$2)*0.8648"))
}
...
writeData(mywb, CC, time_vector, startCol = 14, startCol = 2)
...
# purchases
writeFormula(ivbu, BU, paste0("='", accdrive, ":/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/", settype2, "/[", inputfiles, "]Resettlement Summary'!B30"), startRow = 46, startCol = 9)
writeFormula(ivbu, BU, paste0("='", accdrive, ":/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/", settype2, "/[", inputfiles, "]Resettlement Summary'!C30"), startRow = 46, startCol = 10)
