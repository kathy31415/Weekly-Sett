...
startdate <- documentdata %>%  filter(market2 == "CRM" & run_type == "M4")
startdate <- startdate$bp_start_date %>% unique() %>% as.Date(format = "%Y-%m-%d")
enddate <- documentdata %>% filter(market2 == "CRM" & run_type == "M4")
...
outputfile <- paste0(accdrive, ":/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/M4/20", year, " ", month.as.number, ". ", PTunit, " SEMO ", month, " ", year, " ", settype, " CAPACITY Shadow Settlement - ", initials, ".xlsx")
...
for (i in 1:length(dates)) {
  ...
  writeFormula(mywb, CRMS, paste0("=SUMIF('En & Imp Split'!$A:$A,'CRM Summary'!$A", i+9, ",'En & Imp Split'!E:E)"), startRow = i+9, startCol = 2)
  writeFormula(mywb, CRMS, paste0("=SUMIF('En & Imp Split'!$A:$A,'CRM Summary'!$A", i+9, ",'En & Imp Split'!F:F)"), startRow = i+9, startCol = 3)
  writeFormula(mywb, CRMS, paste0("=-SUMIF('En & Imp Split'!$A:$A,'CRM Summary'!$A", i+9, ",'En & Imp Split'!G:G)"), startRow = i+9, startCol = 4)
  writeFormula(mywb, CRMS, paste0("=SUMIF('En & Imp Split'!$A:$A,'CRM Summary'!$A", i+9, ",'En & Imp Split'!H:H)"), startRow = i+9, startCol = 5)
  ...
  writeFormula(mywb, CRMS, paste0("=SUMIF('En & Imp Split'!$A:$A,'CRM Summary'!$A", i+9, ",'En & Imp Split'!M:M)"), startRow = i+9, startCol = 10)
  writeFormula(mywb, CRMS, paste0("=SUMIF('En & Imp Split'!$A:$A,'CRM Summary'!$A", i+9, ",'En & Imp Split'!N:N)"), startRow = i+9, startCol = 11)
  writeFormula(mywb, CRMS, paste0("=SUMIF('En & Imp Split'!$A:$A,'CRM Summary'!$A", i+9, ",'En & Imp Split'!O:O)"), startRow = i+9, startCol = 12)
  writeFormula(mywb, CRMS, paste0("=SUMIF('En & Imp Split'!$A:$A,'CRM Summary'!$A", i+9, ",'En & Imp Split'!P:P)"), startRow = i+9, startCol = 13)
...
for (ROW in c(2:5, 10:42)) {
  for (COL in c(2:32)) {
    addStyle(mywb, CRMS, style = euro, rows = ROW, cols = COL)
  }
}
...
for (COL in 18:21) {
  addStyle(mywb, CRMS, euro, rows = 12, cols = COL)
}
...
addStyle(mywb, CRMS, euro, rows = 11, cols = c(18:21))
addStyle(mywb, CRMS, euro, rows = 12, cols = c(18:21))
...
temp <- c()
for (i in 2:1492) {
  temp <- c(temp, paste0("=SUMIFS('Consumption Checks'!F:F,'Consumption Checks'!C:C,'SHADOW SETTLED'!C", i, ")*$", LETTERS[which_column], "$2*SUMIFS('CRM REPORT'!M:M,'CRM REPORT'!L:L,'SHADOW SETTLED'!C", i, ",'CRM REPORT'!J:J,\"FQMCC\")"))
}
writeFormula(mywb, SS, temp, startRow = 2, startCol = 5)

temp <- c()
for (i in 2:1492) {
  temp <- c(temp, paste0("=E", i, "*$", LETTERS[which_column], "$3"))
}
writeFormula(mywb, SS, temp, startRow = 2, startCol = 6)

temp <- c()
for (i in 2:1492) {
  temp <- c(temp, paste0("=SUMIFS('Consumption Checks'!F:F,'Consumption Checks'!C:C,'SHADOW SETTLED'!C", i, ")*$", LETTERS[which_column], "$4"))
}
...
temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=('Consumption Checks'!O", ROW, "*SUMIFS('CRM REPORT'!$M:$M,'CRM REPORT'!$L:$L,'En & Imp Split'!$B", ROW+2, ",'CRM REPORT'!$J:$J,\"FQMCC\")*'SHADOW SETTLED'!$", toupper(letters[which_column]), "$2)"))
}
writeFormula(mywb, EIS, temp, startRow = 4, startCol = 5)

temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=('Consumption Checks'!P", ROW, "*SUMIFS('CRM REPORT'!$M:$M,'CRM REPORT'!$L:$L,'En & Imp Split'!$B", ROW+2, ",'CRM REPORT'!$J:$J,\"FQMCC\")*'SHADOW SETTLED'!$", toupper(letters[which_column]), "$2)"))
}
writeFormula(mywb, EIS, temp, startRow = 4, startCol = 6)

temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=('Consumption Checks'!Q", ROW, "*SUMIFS('CRM REPORT'!$M:$M,'CRM REPORT'!$L:$L,'En & Imp Split'!$B", ROW+2, ",'CRM REPORT'!$J:$J,\"FQMCC\")*'SHADOW SETTLED'!$", toupper(letters[which_column]), "$2)"))
}
writeFormula(mywb, EIS, temp, startRow = 4, startCol = 7)

temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=E", ROW+2, "+F", ROW+2, "-G", ROW+2))
}
...
writeFormula(mywb, EIS, temp, startRow = 4, startCol = 10)

temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=C", ROW+2))
}
...
temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=E", ROW+2, "*'SHADOW SETTLED'!$", toupper(letters[which_column]), "$3"))
}
...
temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=M", ROW+2, "+N", ROW+2, "-O", ROW+2))
}
...
writeFormula(mywb, EIS, temp, startRow = 4, startCol = 18)
...
temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("='Consumption Checks'!O", index, "*'SHADOW SETTLED'!$", toupper(letters[which_column]), "$4*VLOOKUP($A", index+2, ",'MO Summary'!$P$8:$S$38,4,0)"))
}
...
temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("='Consumption Checks'!Q", index, "*'SHADOW SETTLED'!$P", toupper(letters[which_column]), "$4*VLOOKUP($A", index+2, ",'MO Summary'!$P$8:$S$38,4,0)"))
}
writeFormula(mywb, EIS, temp, startRow = 4, startCol = 23)

temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=U", index+2, "+V", index+2, "-W", index+2))
}
...
headings <- c("Settlement Date", rep(x = "", times = 3), "HH", "NHH", "PPA Gen", "Total", rep(x = "", times = 4), "HH", "NHH", "PPA Gen", "Total", rep(x = "", times = 4), "HH", "NHH", "PPA Gen", "Total")
...
temp <- c()
temp <- seq(from = 2, by = 1, length.out = 48)
...
temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=VLOOKUP(A", ROW, ",'", accdrive, ":/GeneralAccounts/Settlement/ESB MM/M+4 Resettlement 30 MM/[ESB MASTER MM Messages M+4.xlsx]MM596'!$A:$AY,D", ROW, ",0)"))
}
...
writeData(mywb, CC, time_vector, startCol = 14, startRow = 2)

temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=VLOOKUP($L", ROW, ",'", accdrive, ":/GeneralAccounts/Settlement/ESB MM/M+4 Resettlement 30 MM/[ESB MASTER MM Messages M+4.xlsx]MM595 LA'!$A:$AZ,$M", ROW, ",0)/1000"))
}
writeFormula(mywb, CC, temp, startRow = 2, startCol = 15)

temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=VLOOKUP($L", ROW, ",'", accdrive, ":/GeneralAccounts/Settlement/ESB MM/M+4 Resettlement 30 MM/[ESB MASTER MM Messages M+4.xlsx]MM591 LA'!$A:$AY,$M", ROW, ",0)/1000"))
}
writeFormula(mywb, CC, temp, startRow = 2, startCol = 16)

temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=VLOOKUP($L", ROW, ",'", accdrive, ":/GeneralAccounts/Settlement/ESB MM/M+4 Resettlement 30 MM/[ESB MASTER MM Messages M+4.xlsx]MM598'!$A:$AZ,$M", ROW, ",0)/1000"))
}
...
temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=P", ROW, "+O", ROW, "-Q", ROW, "+R", ROW))
}
...
prevsettype <- "D+4"
prevsettype2 <- "D+4 Initial" 
prevsettype3 <- "INIT"
prevsettype4 <- "D4"
prevmm <- 20

prevcapacityfilepath <- paste0("X:/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/D4/")

prevcapacityfiles <- list.files(prevcapacityfilepath)
prevcapacityfiles <- prevcapacityfiles[grepl(x = prevcapacityfiles, pattern = paste0(month, "|", month2))]
...
writeData(mywb, RS, "CAPACITY M+4 TOTAL", xy = c(1,30))
writeData(mywb, RS, "MO M+4 TOTAL", xy = c(1,39))
...
# FORMULAS
prevfile <- paste0(accdrive, ":/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/D4/[", prevcapacityfiles, "]")
...
openxlsx::conditionalFormatting(mywb, RS, cols = 6, rows = 28, rule = paste0("=F28>0"), type = 'expression', style = posStyle)
openxlsx::conditionalFormatting(mywb, RS, cols = 6, rows = 28, rule = paste0("=F28<0"), type = 'expression', style = negStyle)
openxlsx::conditionalFormatting(mywb, RS, cols = 6, rows = 37, rule = paste0("=F37>0"), type = 'expression', style = posStyle)
openxlsx::conditionalFormatting(mywb, RS, cols = 6, rows = 37, rule = paste0("=F37<0"), type = 'expression', style = negStyle)
...
for (ROW in c(30,39)) {
  for (COL in c(1:6)) {
    addStyle(mywb, RS, createStyle(textDecoration = 'bold', border = 'Bottom', borderStyle = 'Thick', numFmt = "[$???]#,##0.00"), rows = ROW, cols = COL)
  }
}
...
for (ROW in c(2,5,8,12,15,18)) {
  for (COL in c(1:5)) {
    addStyle(mywb, RS, boldStyle, rows = ROW, cols = COL)
  }
}

for (ROW in c(25,27,37)) {
  addStyle(mywb, RS, euro, rows = ROW, cols = 7)
}

for (ROW in c(22,34)) {
  for (COL in c(2:6)) {
    addStyle(mywb, RS, thickbottom, rows = ROW, cols = COL)
  }
}
...
# add M+4 Capacity to Invoice Backup
...
inputfiles <- list.files(paste0(accdrive, ":/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/M4/"))
...
# purchases
writeFormula(ivbu, BU, paste0("='", accdrive, ":/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/M4/[", inputfiles, "]Resettlement Summary'!B30"), startRow = 46, startCol = 9)
writeFormula(ivbu, BU, paste0("='", accdrive, ":/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/M4/[", inputfiles, "]Resettlement Summary'!C30"), startRow = 46, startCol = 10)
writeFormula(ivbu, BU, paste0("='", accdrive, ":/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Capacity/M4/[", inputfiles, "]Resettlement Summary'!D30"), startRow = 46, startCol = 11)
