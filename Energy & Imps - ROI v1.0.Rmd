...
PTunit <- readline(prompt = "Are you settling the new ROI unit? (Y/N): ")
...
if (toupper(PTunit) == "Y" | toupper(PTunit) == "YES") {
  ...
  stop("USE CORRECT R SCRIPT. Stopping...")
}
...
outputfile <- paste0("X:/GeneralAccounts/Settlement/", PTunit, " SEMO Shadow Settlement/Energy + Imp/", settype, "/", PTunit, " SEMO Wk ", weekno, " ", year, " Shadow Settlement ", settype, " KC DON'T USE.xlsx")
...
if (grepl(x = Masterfilepath, pattern = ".xlsx") == FALSE) {
  stop("No copy of MM Master file found. Stopping...")
} else if (length(Masterfilepath) > 1) {
...
if (mm == 30|40) {
  Masterfile <- Masterfile[,c(1,3:52)]
}
...
for (i in 1:length(unique_dates)) {
  ...
  for (j in 2:51) {
  my598[i,j] <- sum(temp[j], na.rm = TRUE)
  }
}
