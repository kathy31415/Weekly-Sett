...
temp <- c()
for (ROW in 2:1489) {
  temp <- c(temp, paste0("=('Consumption Checks'!P", ROW, "*SUMIFS('CRM REPORT'!$M:$M,'CRM REPORT'!$L:$L,'En & Imp Split'!$C", ROW+2, ",'CRM REPORT'!$J:$J,\"FQMCC\")*'SHADOW SETTLED'!$UP$2)*0.8648"))
}
...
for (ROW in c(24:30, 36:39)) {
  addStyle(mywb, RS, createStyle(numFmt = "[$£]#,##0.00", textDecoration = 'bold', border = 'Right', borderStyle = 'thick'), rows = ROW, cols = 7)
}
