...
writeData(wb = mywb, sheet = "Summary", x = "Total", startCol = 2, startRow = startingrow)
writeData(wb = mywb, sheet = "Summary", x = "Statement Total", startCol = 3, startRow = startingrow)
writeData(wb = mywb, sheet = "Summary", x = "Check", startCol = 4, startRow = startingrow)
...
for (row in c(2:5)) {
  addStyle(wb = mywb, sheet = "Summary", createStyle(numFmt = "[$£]#,##0.00"), rows = row, cols = 16:21)
}
...
headings = c("Sales at positive prices", rep.int("", times = 3),
                           "Purchases at negative prices", rep.int("", times = 3),
                           "Sales at negative Prices", rep.int("", times = 3),
                           "Purchases at positive prices")

for (COL in 1:length(headings1)) {
  writeData(mywb, EMS, headings[COL], startRow = 1, startCol = 2)
}

headings <- rep.int(c("Quantity", "", "Commodity", ""), times = 4)
for (COL in 1:length(headings1)) {
  writeData(mywb, EMS, headings[COL], startRow = 2, startCol = 2)
}

headings <- rep.int(c("SU", "AU"), times = 8)
for (COL in 1:length(headings1)) {
  writeData(mywb, EMS, headings[COL], startRow = 3, startCol = 2)
}
