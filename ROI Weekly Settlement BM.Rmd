...
for (col in c(37:40)) {
  for (row in c(4:10, 17:23)) {
    addStyle(wb = mywb, sheet = EIS, createStyle(numFmt = "[$€]#,##0.00"), rows = row, cols = col)
  }
}

# FORMATTING
for (col in c(1,2,10,18,26,42)) {
  addStyle(wb = mywb, sheet = EIS, createStyle(numFmt = "DATE"), rows = 4:339, cols = col)
}
for (col in c(4,12,20,28,44)) {
  addStyle(wb = mywb, sheet = EIS, createStyle(numFmt = "hh:mm:ss"), rows = 4:339, cols = col)
}
for (col in c(5:8,13:16,21:24,19:32,37:40,45)) {
