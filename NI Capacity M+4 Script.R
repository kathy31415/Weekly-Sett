...
temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=VLOOKUP(A", ROW, ",'", accdrive, ":/GeneralAccounts/Settlement/NIE MM/M+4 Resettlement 30 MM/[NI MASTER MM Messages M+4.xlsx]MM596'!$A:$AY,D", ROW, ",0)"))
}
...
temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=VLOOKUP($L", ROW, ",'", accdrive, ":/GeneralAccounts/Settlement/NIE MM/M+4 Resettlement 30 MM/[NI MASTER MM Messages M+4.xlsx]MM595 LA'!$A:$AZ,$M", ROW, ",0)/1000"))
}
writeFormula(mywb, CC, temp, startRow = 2, startCol = 16)

temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=VLOOKUP($L", ROW, ",'", accdrive, ":/GeneralAccounts/Settlement/NIE MM/M+4 Resettlement 30 MM/[NI MASTER MM Messages M+4.xlsx]MM591 LA'!$A:$AY,$M", ROW, ",0)/1000"))
}
writeFormula(mywb, CC, temp, startRow = 2, startCol = 17)

temp <- c()
for (ROW in 2:1491) {
  temp <- c(temp, paste0("=VLOOKUP($L", ROW, ",'", accdrive, ":/GeneralAccounts/Settlement/NIE MM/M+4 Resettlement 30 MM/[NI MASTER MM Messages M+4.xlsx]MM598'!$A:$AZ,$M", ROW, ",0)/1000"))
}
...
prevcapacityfilepath <- "X:/GeneralAccounts/Settlement/PT_500057 SEMO Shadow Settlement/Capacity/D+4 Initial/"
...
for (ROW in c(25,27,37)) {
  addStyle(mywb, RS, sterling, rows = ROW, cols = 7)
}
